...
*/
$('.gameModCon li').hover(function(){//鼠标悬停时
    var the=$(this);										//保存this
    the.css('position','relative');//针对ie7 兼容;
    var pop= the.find('.pop');						//找到隐藏的div
    var parH=$('.gameModCon').height()+240;//父元素的高度加240
    var t=the.position().top;									//获取定位的上和左
    var l=the.position().left;
    var popH=pop.height()+44;							//获取pop的高加44
    var popW=pop.width();
    if(parH<t+popH){												//如果父元素的高小于定位与本身高之和
        pop.css({'top':-pop.height()/2+44,'left':'125px'});//就更改pop的定位，并且显示
    }
    if(l+popW>1000){
